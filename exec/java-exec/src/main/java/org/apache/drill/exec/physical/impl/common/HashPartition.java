...
public class HashPartition implements HashJoinMemoryCalculator.PartitionStat {
  ...
  /**
   *
   * @param isIntersectDistinct
   */
  public void buildContainersHashTableAndHelper(boolean isIntersectDistinct) throws SchemaChangeException {
    ...
    for (int curr = 0; curr < partitionBatchesCount; curr++) {
      ...
      for (int recInd = 0; recInd < currentRecordCount; recInd++) {
       ...
       HashTable.PutStatus putResult;
        try {
          putResult = hashTable.put(recInd, htIndex, hashCode);
        ...
        if (isIntersectDistinct && putResult == HashTable.PutStatus.KEY_PRESENT) {
          continue;
        }
