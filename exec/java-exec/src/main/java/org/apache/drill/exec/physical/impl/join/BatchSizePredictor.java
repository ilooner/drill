...
public interface BatchSizePredictor {
  /**
   * Gets the batchSize computed in the call to {@link #updateStats()}. Returns 0 if {@link #hasData()} is false.
   * @return Gets the batchSize computed in the call to {@link #updateStats()}. Returns 0 if {@link #hasData()} is false.
   * @throws IllegalStateException if {@link #updateStats()} was never called.
   */
  long getBatchSize();

  /**
   * Gets the number of records computed in the call to {@link #updateStats()}. Returns 0 if {@link #hasData()} is false.
   * @return Gets the number of records computed in the call to {@link #updateStats()}. Returns 0 if {@link #hasData()} is false.
   * @throws IllegalStateException if {@link #updateStats()} was never called.
   */
  int getNumRecords();
   ...
   * @throws IllegalStateException if {@link #hasData()} is false or {@link #updateStats()} was not called.
   */
