...
public class HashJoinBatch extends AbstractBinaryRecordBatch<HashJoinPOP> {
  ...
  public IterOutcome innerNext() {
    try {
      ...
      if ( ! buildSideIsEmpty ||  // If there are build-side rows
        ...
        //
        if ( !buildSideIsEmpty && setup && !spilledPartitionsList.isEmpty()) {
  ...
  public void executeBuildPhase() throws SchemaChangeException, ClassTransformationException, IOException {
    ...
    while (moreData) {
      ...
      case OK:
        ...
        if (!firstDataBuild && currentRecordCount > 0) {
          firstDataBuild = true;
          ...
          if (canSpill) {
            ...
            if (hasProbeData) {
              ...
              buildCalc.initialize(firstCycle, // We auto tune only on the first cycle
                firstCycle, // We only reserve the hash values on the first cycle. After the first cycle they are included.
                buildBatch,
                probeBatch,
                buildJoinColumns,
                allocator.getLimit(),
        ...
        for (int ind = 0; ind < currentRecordCount; ind++) {
          ...
          if ( pos + 1 == RECORDS_PER_BATCH ) {
            if (canSpill && hasProbeData) {
              ...
              if (spill && !isSpilled) {
                buildCalc.spill(currPart);
              }
            } else {
              completeAnInnerBatch(currPart, true, false);
            }
          }
        }
    ...
    for (int currPart = 0; currPart < numPartitions; currPart++) {
      ...
      if ( recordCount == 0 ) { // in case the current is empty
        ...
        boolean spilled = isSpilled(currPart);

        if (!spilled && canSpill && hasProbeData && firstDataBuild) {
          buildCalc.addBatchToPartition(currPart, currentBatches[currPart]);
        }
        // add the current/last container to its partition, spill if not pristine
        completeAnInnerBatch(currPart,false, spilled);
      }
  ...
  public void cleanup() {
    if ( buildSideIsEmpty || !setup ) { return; } // not set up; nothing to clean
  ...
  private void freeInMemData() {
    if ( buildSideIsEmpty || !setup ) { return; }
    for (int part = 0; part < numPartitions; part++) {
