...
import org.apache.drill.exec.store.parquet.columnreaders.VLOverflowReader.FieldOverflowStateImpl;
...
public final class VLColumnBulkInput<V extends ValueVector> implements VLBulkInput<VLBulkEntry> {
  private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(VLColumnBulkInput.class);
  ...
  /** An indicator on whether this column is a sparse column */
  ...
  VLColumnBulkInput(VarLengthValuesColumn<V> _parentInst,
    ...
    this.callback                    = new VLColumnBulkInputCallback(this);
    ...
    this.isSparseDecimal                    = BatchSizingMemoryUtil.isSparseBoolean(this.parentInst.valueVec.getField().getType());
  ...
  public final VLBulkEntry next() {
    ...
    final VLColumnBulkEntry result = buffPagePayload.getEntry(remaining);
  ...
  public static int getMinVLColumnMemorySize() {
    ...
    return VLBulkPageReader.BUFF_SZ * 2;
  }
  ...
  private final void setBufferedPagePayload() {
    ...
    if (parentInst.pageReader.hasPage() && oprReadState.numPageFieldsProcessed < parentInst.pageReader.currentPageCount) {
      ...
      if (buffPagePayload == null) {
        buffPagePayload = new VLBulkPageReader(pageInfo, columnPrecInfo, callback, fieldOverflowStateContainer);
      ...
      if (buffPagePayload == null) {
        buffPagePayload = new VLBulkPageReader(null, columnPrecInfo, callback, fieldOverflowStateContainer);
  ...
  private boolean batchConstraintsReached() {
    ...
    final int maxNumRecordsInChunk = VLBulkPageReader.BUFF_SZ / BatchSizingMemoryUtil.INT_VALUE_WIDTH;
    ...
    if (this.parentInst.valueVec.getField().isNullable()) {
      return batchConstraintsReached(
        ...
        VLBulkPageReader.BUFF_SZ                                       // max "data"    space within a chunk
      );
  ...
  private boolean batchConstraintsReached(int newBitsMemory, int newOffsetsMemory, int newDataMemory) {
    ...
    // No need to subject sparse decimal values to batching constraints as internally they are
    // associated with a fixed length storage (they are also accessed last to avoid overflow
    // handling because of previous VL columns).
    if (isSparseDecimal) {
      return false;
    }
  ...
  final static class VLColumnBulkInputCallback {
    /** Parent instance */
    final VLColumnBulkInput<? extends ValueVector> parentInst;
    /** Page reader object */
    PageReader pageReader;

    VLColumnBulkInputCallback(VLColumnBulkInput<? extends ValueVector> parentInst) {
