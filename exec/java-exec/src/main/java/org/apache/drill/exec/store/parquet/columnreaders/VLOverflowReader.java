 ...
 */
public final class VLOverflowReader extends VLAbstractEntryReader {
   ...
   */
  VLOverflowReader(ByteBuffer _buffer,
  ...
  VLColumnBulkEntry getEntry(int valuesToRead) {
    ...
    final int maxDataSize                      = VLBulkPageReader.BUFF_SZ;
    ...
    for ( ; idx < numAvailableValues; idx++, currValueIdx++) {
      ...
      if (!isNullable || overflowDataCache.getNullable(currValueIdx) == 1) {
        final int data_len = overflowDataCache.getDataLength(currValueIdx);

        if ((totalDataLen + data_len) > maxDataSize) {
          break;
        }

        totalDataLen      += data_len;
    ...
    if (idx == 0) {
      return handleLargeEntry(maxDataSize, firstValueDataOffset, totalDataLen);
    }
  ...
  private VLColumnBulkEntry handleLargeEntry(int maxDataSize, int firstValueDataOffset, int totalDataLen) {
...
// ----------------------------------------------------------------------------
  ...
  final static class OverflowDataCache {
    ...
    private static final int MAX_NUM_VALUES = (VLBulkPageReader.BUFF_SZ / (BatchSizingMemoryUtil.INT_VALUE_WIDTH + 1)) - 1;
    ...
    private OverflowDataCache(ByteBuffer buffer, FieldOverflowDefinition overflowDef) {
      this.buffer_array        = buffer.array();
    ...
    private int getDataLength(int valueIdx) {
      ...
      return VLOverflowReader.getInt(buffer_array, cacheIdx2) - VLOverflowReader.getInt(buffer_array, cacheIdx1);
    }
    ...
    void loadNullable() {
      ...
      overflowDef.buffer.getBytes(firstCachedValueIdx, buffer_array, 0, numCachedValues);
    }
    ...
    void loadOffsets() {
      ...
      overflowDef.buffer.getBytes(sourceIdx, buffer_array, targetIdx, (numCachedValues + 1) * BatchSizingMemoryUtil.INT_VALUE_WIDTH);
    }
